...
func TestVerifyPassword(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := VerifyPassword(tt.password, tt.hashedPassword)
...
func TestGenerateSalt(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		if len(salt) != SaltSize {
			t.Errorf("GenerateSalt() неправильный размер соли: получили %d, ожидали %d", len(salt), SaltSize)
		}
...
func TestDeriveKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			key := DeriveKey(password, salt, tt.keyLen)
	...
	key3 := DeriveKey("differentpassword", salt, 32)
	...
	key4 := DeriveKey(password, differentSalt, 32)
...
func TestGenerateRandomBytes(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		seq, err := GenerateRandomBytes(32)
		if err != nil {
...
func TestCreateMasterKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			key, salt, err := CreateMasterKey(tt.password)
			...
			if len(salt) != SaltSize {
				t.Errorf("Неправильный размер соли: получили %d, ожидали %d", len(salt), SaltSize)
			}

			// Проверяем, что можно восстановить тот же ключ
			restoredKey := RestoreMasterKey(tt.password, salt)
