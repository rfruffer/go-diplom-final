...
func TestTokenManager_GenerateTokens(t *testing.T) {
	...
	tm := NewTokenManager(signingKey, 15*time.Minute, 24*time.Hour, "test-issuer")
...
func TestTokenManager_ExpiredToken(t *testing.T) {
	...
	tm := NewTokenManager(signingKey, 1*time.Millisecond, 24*time.Hour, "test-issuer")
...
func TestNewTokenManagerFromConfig(t *testing.T) {
	...
	tm := NewTokenManagerFromConfig(config)

	assert.NotNil(t, tm)
	assert.Equal(t, config.AccessTokenTTL, tm.accessTokenTTL)
	assert.Equal(t, config.RefreshTokenTTL, tm.refreshTokenTTL)
	assert.Equal(t, config.Issuer, tm.issuer)
}