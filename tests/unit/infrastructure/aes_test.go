...
func TestNewAESCipher(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				if err == nil {
					t.Errorf("crypto.NewAESCipher() ожидалась ошибка, но получили nil")
				}
				if cipher != nil {
					t.Errorf("crypto.NewAESCipher() ожидался nil, но получили cipher")
				}
			} else {
				if err != nil {
					t.Errorf("crypto.NewAESCipher() неожиданная ошибка: %v", err)
				}
				if cipher == nil {
					t.Errorf("crypto.NewAESCipher() ожидался cipher, но получили nil")
				}
			}
		})
	}
...
func TestGenerateKey(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		if err != nil {
			t.Fatalf("crypto.GenerateKey() ошибка: %v", err)
		}

		// Проверяем длину ключа
		if len(key) != 32 {
			t.Errorf("crypto.GenerateKey() неправильная длина ключа: получили %d, ожидали 32", len(key))
		}
	...
	for i := 0; i < len(keys); i++ {
		for j := i + 1; j < len(keys); j++ {
			if bytes.Equal(keys[i], keys[j]) {
				t.Errorf("crypto.GenerateKey() сгенерировал одинаковые ключи")
			}
		}
	}
}
...
func TestEncryptDecryptWithPassword(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Шифруем
			ciphertext, err := crypto.EncryptWithPassword(tt.plaintext, tt.password)
			if tt.wantErr {
				if err == nil {
					t.Errorf("crypto.EncryptWithPassword() ожидалась ошибка")
				}
				return
			}

			if err != nil {
				t.Errorf("crypto.EncryptWithPassword() неожиданная ошибка: %v", err)
				return
			}

			// Расшифровываем с правильным паролем
			decrypted, err := crypto.DecryptWithPassword(ciphertext, tt.password)
			if err != nil {
				t.Errorf("crypto.DecryptWithPassword() неожиданная ошибка: %v", err)
				return
			}
			...
			_, err = crypto.DecryptWithPassword(ciphertext, "wrong_password")
			if err == nil {
				t.Errorf("crypto.DecryptWithPassword() должен возвращать ошибку для неправильного пароля")
			}
		})
	}
}
